...
- **Demo clone cleanup (COOLIFY-DEMO-3)**: removed `demo-vendome.welcomeparis.hotelmanager.fr` and `demo-admin.vendome.welcomeparis.hotelmanager.fr` from the Web clone FQDNs (they did not match the seed slug and caused 404 console errors). Removed duplicate `SEED_DEMO_ENABLED` and `SEED_DEMO_SECRET` env vars on the API clone. **Added canonical FQDNs `demo-paris-local.welcomeparis.hotelmanager.fr` (Guest) and `admin-demo-paris-local.welcomeparis.hotelmanager.fr` (Reception) to the Web clone** so that opening either URL directly deduces the correct slug via `tenant.ts`. Fallback path-based URL `https://demo.hotelmanager.fr/h/demo-paris-local/welcome` still works. Official demo URLs documented in `docs/COOLIFY_DEMO_ISOLATION.md` section 12.
