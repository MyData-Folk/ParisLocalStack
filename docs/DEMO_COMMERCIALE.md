...
Statut au 2026-06-06 (COOLIFY-DEMO-1 + COOLIFY-DEMO-2 + COOLIFY-DEMO-3 + COOLIFY-DEMO-4) : un **clone dedie** `paris-local-demo` est operationnel et seede :
...
- **NE PAS OUVRIR** `https://demo.hotelmanager.fr/` : cette racine a ete retirees du Web clone en COOLIFY-DEMO-4 car elle faisait deduire `slug=demo` (label non plateforme dans `tenant.ts:36`) et provoquait des 404 console navigateur sur les appels API. Si l'utilisateur l'ouvre par megarde, c'est qu'il utilise encore un ancien signet ; le rediriger vers `https://demo-paris-local.welcomeparis.hotelmanager.fr/`.
